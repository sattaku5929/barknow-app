...
async function accessTokenForCoach(coachId: string) {
  ...
  const payload = await response.json();
  if (!response.ok) {
    await admin.from("wt_google_calendar_connections").update({ sync_status: "error", last_error: String(payload.error_description ?? payload.error), updated_at: new Date().toISOString() }).eq("coach_id", coachId);
...
export async function processCalendarSyncJobs(options: { coachId?: string; sessionId?: string; limit?: number } = {}) {
  const admin = serviceSupabase();
  let query = admin.from("wt_calendar_sync_jobs").select("id,session_id,coach_id,action,attempts").in("status", ["pending", "failed"]).lte("run_after", new Date().toISOString()).order("created_at").limit(options.limit ?? 5);
  ...
  for (const job of jobs ?? []) {
    await admin.from("wt_calendar_sync_jobs").update({ status: "processing", attempts: job.attempts + 1, updated_at: new Date().toISOString() }).eq("id", job.id);
    try {
      ...
      if (job.action === "cancel" || session.status === "cancelled") {
        ...
        const meetUrl = event.hangoutLink ?? event.conferenceData?.entryPoints?.find((item: { entryPointType: string }) => item.entryPointType === "video")?.uri ?? null;
        await admin.from("wt_online_sessions").update({ google_event_id: event.id, google_event_etag: event.etag, meet_url: meetUrl, calendar_sync_status: "synced", calendar_sync_error: null }).eq("id", session.id);
      }
